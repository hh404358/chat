package com.hahaha.platform.modules.push.service.impl;
...
import com.hahaha.platform.modules.push.enums.PushBodyEnum;
...
import com.hahaha.platform.modules.push.vo.*;
import com.hahaha.platform.modules.ws.BootWebSocketHandler;
...
public class ChatPushServiceImpl implements ChatPushService {
    ...
    private void doMsg(PushParamVo from, PushParamVo group, PushMsgEnum msgType) {
        ...
        if (top != null) {
            pushMsgVo.setTop(top.getCode());//消息顶置
        }
        YesOrNoEnum disturb = from.getDisturb();
        if (disturb != null) {
            pushMsgVo.setDisturb(disturb.getCode());//消息免打扰
        }
    ...
    private void doNotice(Long userId, PushParamVo paramVo, PushNoticeEnum pushNotice) {
        ...
        if(pushNotice.equals(PushNoticeEnum.FRIEND_APPLY)){
